...
export async function GET(request, { params }) {
  ...
  const films = await queryDatabricks(`
    ...
    FROM (
      SELECT film_id, person_id, job, NULL AS character FROM workspace.silver.film_crew
      WHERE person_id = ${id}
        AND job IN ('Director','Screenplay','Writer',
                    'Director of Photography','Original Music Composer','Producer')
      UNION ALL
      SELECT film_id, person_id, NULL AS job, character FROM workspace.silver.film_cast
      WHERE person_id = ${id}
    ) c
    JOIN workspace.silver.movies m ON c.film_id = m.film_id
